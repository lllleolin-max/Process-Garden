...
export function ProcessExplorer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="modal-backdrop process-list-backdrop" data-overlay-root="modal" data-state={overlay.state} aria-hidden={!open} inert={!open}>
    <section className="process-explorer" ref={overlay.surfaceRef} role="dialog" aria-modal="true" aria-labelledby="process-list-title" tabIndex={-1}>
      ...
      <footer><button className="process-list-control" aria-disabled={currentPage === 0} onClick={() => { if (currentPage > 0) turnPage(currentPage - 1); }}>{t("processList.previous")}</button><span>{t("processList.page", { page: currentPage + 1, pages: pageCount })}</span><button className="process-list-control" aria-disabled={currentPage + 1 >= pageCount} onClick={() => { if (currentPage + 1 < pageCount) turnPage(currentPage + 1); }}>{t("processList.next")}</button></footer>
    </section>
  </div>;
}
